/* ============================================
   MAIN.JS — Data rendering, terminal & theme
   ============================================ */
...
const SVG_ICONS = {
  ...
  building: '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="4" y="3" width="16" height="18" rx="1"/><line x1="9" y1="7" x2="9" y2="7"/><line x1="15" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="9" y2="11"/><line x1="18" y1="15" x2="18" y2="15"/></svg>',
  ...
  cert: '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 15l-2 5l3-2l3 2l-2-5"/><circle cx="12" cy="9" r="6"/></svg>'
};
...
const App = {
  ...
  init() {
    try {
      if (typeof SITE_DATA !== 'undefined') {
        ...
        console.error('الملف data.js غير محمل أو المتغير SITE_DATA غير موجود!');
        return;
      }
  ...
  lang() { 
    return (window.Language && Language.current) ? Language.current : 'en'; 
  },
  ...
  renderHeader() {
    if (!this.data.profile || !this.data.profile.identity) return;
    const id = this.data.profile.identity;
    
    // مطابقة العناصر بواسطة الكلاسات أو IDs المتاحة في index.html
    const nameEl = document.querySelector('.hero-name') || document.getElementById('hero-name');
    const titleEl = document.querySelector('.hero-title-primary') || document.getElementById('hero-title');
    const secTitleEl = document.querySelector('.hero-title-secondary');
    const taglineEl = document.querySelector('.hero-tagline') || document.getElementById('hero-tagline');
    const summaryEl = document.querySelector('.hero-summary') || document.getElementById('hero-summary');

    if (nameEl) nameEl.textContent = this.tr(id.name);
    if (titleEl) titleEl.textContent = this.tr(id.headline);
    if (secTitleEl && id.secondaryTitle) secTitleEl.textContent = this.tr(id.secondaryTitle);
    if (taglineEl) taglineEl.textContent = this.tr(id.tagline);
    if (summaryEl) summaryEl.textContent = this.tr(id.summary);
  },
  ...
  renderCertifications() {
    ...
    if (!container || !this.data.profile || !this.data.profile.certifications) return;
    container.innerHTML = this.data.profile.certifications.map(cert => `
      <div class="cert-card">
        <div class="cert-icon">${SVG_ICONS.cert}</div>
        <div class="cert-info">
  ...
  renderFooter() {
    ...
    if (cp) {
      ...
      if (window.Language && typeof Language.t === 'function') {
        cp.textContent = Language.t('footer.copyright').replace('{year}', year);
      } else {
        cp.textContent = `© ${year} Moatasem. All rights reserved.`;
      }
    }
  },
  ...
  initTerminal() {
    ...
    let lines = [];
    if (window.Language && typeof Language.t === 'function') {
      lines = Language.t('terminal');
    }
    
    if (!lines || !Array.isArray(lines)) {
      lines = [
        "System initialization sequence completed.",
        "Status: Network Operational.",
        "Ready for administrative commands."
      ];
    }

    const outputContainer = document.getElementById('terminal-output') || body;
...
// --- إدارة الوضع الداكن والفاتح (Dark / Light Mode) ---
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. جلب الوضع المحفوظ أو اعتماده كـ dark بشكل افتراضي
  const savedTheme = localStorage.getItem('theme') || 'dark';
  htmlElement.setAttribute('data-theme', savedTheme);
  updateThemeIcon(savedTheme);

  // 2. ربط حدث الضغط على الزر
  if (themeToggleBtn) {
    themeToggleBtn.addEventListener('click', (e) => {
      ...
      // تطبيق الثيم الجديد وحفظه في LocalStorage
      htmlElement.setAttribute('data-theme', newTheme);
      localStorage.setItem('theme', newTheme);
  ...
  // دالة لتحديث شكل الأيقونة (شمس / قمر)
  function updateThemeIcon(theme) {
    if (!themeIcon) return;
    if (theme === 'light') {
      themeIcon.className = 'fas fa-sun';
